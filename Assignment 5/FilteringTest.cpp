...
DynamicArray FilteringTest::arr;
...
void FilteringTest::testFilteringPrice() {
	...
	DynamicArray filteredArr = p1.filter(arr);
	assert(filteredArr.getLength() == 2);
	assert(filteredArr.get(0) == o1);
	assert(filteredArr.get(1) == o2);

	p1.set_price(99);
	filteredArr = p1.filter(arr);
	assert(filteredArr.getLength() == 0);

	p1.set_price(1101);
	filteredArr = p1.filter(arr);
	assert(filteredArr.getLength() == arr.getLength());
	for (int i = 0; i < filteredArr.getLength(); i++) assert(filteredArr.get(i) == arr.get(i));

}

void FilteringTest::testFilteringType() {
	FilteringType t(circuit);
	DynamicArray filteredArr = t.filter(arr);
...
void FilteringTest::testFilteringPriceAndType() {
	...
	DynamicArray filteredArr = tp1.filter(arr);
	assert(filteredArr.getLength() == 1);
	assert(filteredArr.get(0) == o1);
